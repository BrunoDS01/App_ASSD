...
def predictWithU_Net(songPath):
    ...
    drumsSpec = None
    # load saved model
    model_vocals = keras.models.load_model('src/models/vocal_PRUEBA_fin.h5')
    model_drums = keras.models.load_model('src/models/vocal_PRUEBA_drums.h5')
    ...
    for START in range(0, timeFrames, 128):
        ...
        y_vocals_p = model_vocals.predict(X, batch_size=32)
        y_vocals = np.vstack((np.zeros((128)), y_vocals_p.reshape(512, 128)))

        y_drums_p = model_drums.predict(X, batch_size=32)
